...
Option parse_option(std::vector<std::string> const &args)
{
    ...
    for(; i < argc; ++i)
    {
        if(args[i].compare("-d") == 0)
        {
			...
			CART::max_depth = std::stoi(args[++i]);//树的最大深度
        }
        else if(args[i].compare("-t") == 0) //树的的数目
        {
            if(i == argc-1)
                throw std::invalid_argument("invalid command");
            opt.nr_tree = std::stoi(args[++i]);
        }
        else if(args[i].compare("-s") == 0)//使用几个线程进行计算
        {
            if(i == argc-1)
                throw std::invalid_argument("invalid command");
...
int main(int const argc, char const * const * const argv)
{
    ...
    Problem const Tr = read_data(opt.Tr_path, opt.TrS_path);//读取训练数据
    Problem const Va = read_data(opt.Va_path, opt.VaS_path);//读取验证数据
    std::cout << "done\n" << std::flush;
